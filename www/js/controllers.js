...
.controller('documentsCtrl', function($scope, $state, $cordovaCamera, $cordovaDialogs, $rootScope, $localStorage) {
  ...
  if($scope.$storage.documents){
      $scope.toupload_documents = $scope.$storage.documents;
  }
...
.controller('documentDetailsCtrl', function($scope, $state, $cordovaCamera, $ionicModal, $stateParams, $rootScope, $cordovaDialogs, $localStorage,
  ...
  if($ionicHistory.backView().stateName == 'menu.documents'){
  ...
  else if($scope.$storage.documents){
      $scope.documentDetail.passportNum = $scope.$storage.documents[0].passportNum;
      $scope.documentDetail.surname = $scope.$storage.documents[0].surname;
      $scope.documentDetail.givenName = $scope.$storage.documents[0].givenName;
      $scope.documentDetail.nationality = $scope.$storage.documents[0].nationality;
      $scope.documentDetail.birthDate = $scope.$storage.documents[0].birthDate;
      $scope.documentDetail.sex = $scope.$storage.documents[0].sex;
      $scope.documentDetail.authority = $scope.$storage.documents[0].authority;
      $scope.documentDetail.expiryDate = $scope.$storage.documents[0].expiryDate;
  }
  ...
  $scope.save = function() {
      if(!$scope.$storage.documents){
          $scope.$storage.documents = [];
      }
      $scope.$storage.documents.push($scope.documentDetail);
  };
